...
export default function ViewUnloadingListPage() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/unloadinglist/track/${id}`, {
        headers: {
      ...
      if (!response.ok || !data.flag) {
        showError(data.message || "Error occurred while fetching unloading list", "Error");
        setIsLoading(false);
        return;
      }
      ...
      showError("Failed to fetch unloading list details", "Error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      {/* Unloading List Details Section */}
      <Box
        sx={{
        ...
        {isMobile ? (
          <Box sx={{ p: 2, pt: 0 }}>
            {isLoading ? (
              ...
              <Box sx={{ textAlign: "center", py: 4, color: colors?.textSecondary || "#64748b" }}>No parcels in this unloading list.</Box>
            )}
          </Box>
        ) : (
          /* Desktop Table View */
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={4} align="center" sx={{ py: 3, color: colors?.textSecondary || "#64748b" }}>
                      No parcels in this unloading list.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
