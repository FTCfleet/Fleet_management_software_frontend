  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/loadinglist/track-all/${selectedDate}`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch loading lists");
      }

      const data = await response.json();
      setLoadingLists(data.body || []);
      ...
      alert("Failed to fetch loading lists. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {isLoading ? (
                ...
                <TableRow key={entry._id} hover>
                  ...
                  <TableCell sx={{ textAlign: "center" }}>
                    <IconButton
                      ...
                      onClick={() => navigate(`/user/view/loading-list/${entry._id}`)}
                    >
              ...
              <TableRow>
                <TableCell colSpan="8" align="center" sx={{ color: colors?.textSecondary || "#7D8695" }}>
                  No loading lists found for the selected date.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
export default AllLoadingListPage;
